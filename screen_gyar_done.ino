...
void setup(void) {
  ...
  pinMode(awninginPin, OUTPUT);
  pinMode(awningstopPin, OUTPUT);
  pinMode(awningoutPin, OUTPUT);
...
void loop() {
  ...
  if (!ts.bufferEmpty())
  {
    ...
    if ((x >= 80) && (x <=170)) {
        if ((y >= 125) && (y <= 115)){
          digitalWrite(awninginPin, HIGH);
        }
    }
    if ((x >= 195) && (x <=285)) {
        if ((y >= 125) && (y <= 215)){
          digitalWrite(awningstopPin, HIGH);
        }
    }
    if ((x >= 310) && (x <=400)) {
        if ((y >= 125) && (y <= 215)){
          digitalWrite(awningoutPin, HIGH);
        }
    }   
  }
  }
